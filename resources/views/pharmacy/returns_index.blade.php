...
<div class="container mx-auto px-4 py-8">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        ...
        <div class="flex flex-wrap items-center gap-2">
            <a href="{{ route('pharmacy.returns.create') }}" class="inline-flex items-center bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 min-h-11 rounded">
            ...
            <tbody>
                @forelse($records as $r)
                    <tr class="border-t border-gray-200">
                        <td class="px-4 py-3 font-medium whitespace-nowrap">{{ $r->inventoryItem->medicine->medicine_name }}</td>
                        <td class="px-4 py-3 whitespace-nowrap">
                            <span class="px-2 py-1 rounded text-xs {{ $r->type === 'recall' ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700' }}">
                                {{ ucfirst($r->type) }}
                            </span>
                        </td>
                        <td class="px-4 py-3 whitespace-nowrap">{{ $r->quantity }}</td>
                        ...
                        <td class="px-4 py-3 whitespace-nowrap">
                            ...
                            <form method="POST" action="{{ route('pharmacy.returns.status', $r->id) }}" class="inline">
                                @csrf
                                <select name="status" onchange="this.form.submit()" class="border border-gray-300 rounded px-2 py-1.5 text-sm {{ $colors[$r->status] ?? '' }}">
                        ...
                        <td class="px-4 py-3 whitespace-nowrap">{{ \Carbon\Carbon::parse($r->created_at)->format('M d, Y') }}</td>
                    </tr>
                @empty
