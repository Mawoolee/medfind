...
<div class="container mx-auto max-w-6xl px-4 py-8">
    ...
    <div class="mb-6 grid grid-cols-1 gap-4 rounded-xl border border-gray-200 bg-white p-5 shadow-sm sm:grid-cols-3">
        <div>
            ...
            <p class="mt-1 break-all text-sm font-medium text-gray-900">{{ $saleReference }}</p>
        </div>
        <div>
            <p class="text-xs font-semibold uppercase tracking-wide text-gray-500">Server Date &amp; Time</p>
            <p class="mt-1 text-sm font-medium text-gray-900">{{ $serverTimestamp->format('M d, Y g:i A') }}</p>
        </div>
        <div>
    ...
    @else
        ...
        <form method="POST" action="{{ route('pharmacy.sales.store') }}" class="space-y-6" novalidate>
            ...
            <button type="button" class="inline-flex items-center rounded-lg border border-blue-600 px-4 py-2 text-sm font-semibold text-blue-700 hover:bg-blue-50" data-add-row>
            ...
            <div class="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                <a href="{{ route('pharmacy.dashboard') }}" class="rounded-lg border border-gray-300 px-5 py-2.5 text-center text-sm font-semibold text-gray-700 hover:bg-gray-50">Cancel</a>
                <button type="submit" class="rounded-lg bg-blue-700 px-5 py-2.5 text-sm font-semibold text-white hover:bg-blue-800">Record Sale</button>
            </div>
        </form>
