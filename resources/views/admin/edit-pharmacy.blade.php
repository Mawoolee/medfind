...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-white rounded-xl shadow-lg p-5 sm:p-6 max-w-2xl mx-auto">
        <form action="{{ route('admin.pharmacy.update', $pharmacy->id) }}" method="POST">
            ...
            @php
                $lat  = old('latitude',  session('admin_edit_pharmacy_' . $pharmacy->id . '.location.latitude',  $pharmacy->latitude));
                $lng  = old('longitude', session('admin_edit_pharmacy_' . $pharmacy->id . '.location.longitude', $pharmacy->longitude));
                $addr = old('location_address', session('admin_edit_pharmacy_' . $pharmacy->id . '.location.address'));
                $hasLocation = $lat !== null && $lat !== '' && $lng !== null && $lng !== '';
            @endphp
            <div class="mb-4">
                <label class="block text-gray-700 text-sm font-medium mb-2">Pharmacy Location</label>

                {{-- Confirmed / existing location state --}}
                @if ($hasLocation)
                <div class="location-confirmed-map relative mb-2 overflow-hidden rounded-xl border border-green-300 px-3 py-2.5">
                    <a href="{{ route('admin.pharmacy.location.edit', $pharmacy->id) }}"
                    ...
                    <div class="relative z-10">
                        ...
                        @elseif ($lat && $lng)
                            <p class="mt-1 pr-8 text-xs text-gray-400 leading-snug">{{ $lat }}, {{ $lng }}</p>
                        @endif
                    </div>
                </div>
                @else
                <a href="{{ route('admin.pharmacy.location.edit', $pharmacy->id) }}"
                   class="group flex w-full flex-col items-center justify-center gap-1.5 rounded-xl border-2 border-dashed border-blue-400/40 bg-blue-50/30 px-4 py-6 text-center cursor-pointer transition hover:border-blue-500 hover:bg-blue-50">
            ...
            <div class="mb-4">
                ...
                <select id="user_id" name="user_id" class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-base focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            ...
            <div class="mb-4">
                ...
                <select id="status" name="status" class="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-base focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            ...
            <div class="flex flex-col sm:flex-row gap-3">
                ...
                <a href="{{ route('admin.pharmacies') }}" class="inline-flex items-center justify-center bg-gray-300 hover:bg-gray-400 text-gray-800 px-6 py-2.5 rounded-xl transition duration-200 w-full sm:w-auto">
                    Cancel
                </a>
            </div>
        </form>
