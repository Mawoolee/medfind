...
<div class="container mx-auto px-4 py-8">
    <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex gap-3">
            <a href="{{ route('admin.medicine.add') }}" class="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg transition duration-200">
...
@if(session('success'))
    ...
    <!-- Search & Filter -->
    <form method="GET" action="{{ route('admin.medicines') }}" class="bg-white rounded-lg shadow-lg p-4 mb-6 flex flex-wrap gap-3 items-end">
        ...
        <a href="{{ route('admin.medicines') }}" class="text-gray-500 hover:text-gray-700 text-sm px-2 py-2">Reset</a>
    </form>
        ...
        <div class="p-6">
            <div class="overflow-x-auto">
                <table class="w-full">
                    <thead>
                        <tr class="bg-gray-50">
                            <th class="px-4 py-3 text-left text-gray-600">Medicine Name</th>
                            ...
                            <th class="px-4 py-3 text-left text-gray-600">Prescription</th>
                    ...
                    <tbody>
                        @forelse($medicines as $medicine)
                            <tr class="border-t border-gray-200">
                                <td class="px-4 py-3 font-semibold">{{ $medicine->medicine_name }}</td>
                                ...
                                <td class="px-4 py-3">
                                    @if($medicine->requiresPrescription)
                                        <span class="px-2 py-1 rounded text-xs bg-red-100 text-red-700">Rx Required</span>
                                    @else
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex gap-3">
                                        <a href="{{ route('admin.medicine.edit', $medicine->id) }}" class="text-blue-600 hover:text-blue-800">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form action="{{ route('admin.medicine.delete', $medicine->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" onclick="return confirm('Delete this medicine?')" class="text-red-600 hover:text-red-800">
