...
<div class="container mx-auto px-4 py-8">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        ...
        <div class="flex items-center gap-4">
            <a href="{{ route('admin.medicine.add') }}" class="inline-flex items-center justify-center bg-purple-600 hover:bg-purple-700 text-white px-4 py-2.5 rounded-xl transition duration-200">
...
@if(session('success'))
    ...
    <!-- Search & Filter -->
    <form method="GET" action="{{ route('admin.medicines') }}" class="bg-white rounded-xl shadow-lg p-4 mb-6 flex flex-col sm:flex-row flex-wrap gap-3 sm:items-end">
        <div class="flex-1 min-w-[200px]">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by name, manufacturer, category..."
                   class="w-full border border-gray-300 rounded-xl px-3 py-2.5 text-base sm:text-sm">
        </div>
        <div>
            <label class="block text-sm font-semibold text-gray-600 mb-1">Category</label>
            <select name="category" class="w-full sm:w-auto border border-gray-300 rounded-xl px-3 py-2.5 text-base sm:text-sm">
                            ...
                            <tr class="border-t border-gray-200">
                                ...
                                <td class="px-4 py-3">
                                    @if($medicine->requiresPrescription)
                                        <span class="px-2 py-1 rounded-xl text-xs bg-red-100 text-red-700 whitespace-nowrap">Rx Required</span>
                                    @else
                                        <span class="px-2 py-1 rounded-xl text-xs bg-green-100 text-green-700">OTC</span>
                                    @endif
                                </td>
                                <td class="px-4 py-3">
