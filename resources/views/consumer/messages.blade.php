        ...
        <div class="flex-1 overflow-y-auto">
            ...
            @forelse($grouped as $pharmacyId => $thread)
                @php
                    ...
                    $unread = $thread->where('sender', 'pharmacy')->whereNull('consumer_read_at')->count();
                    $lastText = $lastMsg->sender === 'pharmacy' ? $pharmacy->pharmacy_name . ': ' . Str::limit($lastMsg->message, 25) : 'You: ' . Str::limit($lastMsg->message, 30);
                @endphp
                    ...
                    <div class="flex items-center gap-3 px-5 py-3 cursor-pointer hover:bg-white/5 transition border-b border-white/5"
                        ...
                        <div class="flex items-center gap-2 flex-shrink-0">
                            ...
                            <button @click.stop="menuOpen = !menuOpen"
                                    class="text-gray-500 hover:text-white opacity-0 group-hover:opacity-100 transition p-1">
...
function openConversation(pharmacyId) {
    ...
    fetch('/consumer/messages/' + pharmacyId + '/mark-read', {
        method: 'POST',
      ...
      .then(function(data) {
        if (data.success) {
            // Remove the unread badge from this conversation
            if (item) {
